...
import { InsertUser, users, chatRooms, messages, InsertChatRoom, InsertMessage } from "../drizzle/schema";
...
export async function getChatRoomByRoomId(roomId: string) {
  const db = await getDb();
  if (!db) {
    return undefined;
  }
  const result = await db.select().from(chatRooms).where(eq(chatRooms.roomId, roomId)).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
